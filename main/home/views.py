...
from PRODUCT.models import Products,Order,Cart
  ...
  if request.method=='POST':
    ...
    if query:
       ...
       if len(query) >= 100:
           ...
           print('searching ...working ')   
           search_results=Products.objects.filter(description__icontains=query)
    ...
    try:
        cart_number=Cart.objects.get(auth_user=request.user)

        print("Home Page ! ")
        if cart_number:
            return render(request,'homeapp_templates/homepage.html',{'all_p':all_p,'cart_number':cart_number})
